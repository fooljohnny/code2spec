...
- **方法体片段**：最多 2000 字
- **完整调用链代码**：最多 12000 字（含多级方法、Javadoc、注解）

粗略估算：单次请求 **输入约 3000~8000 tokens**（视调用链深度而定）。
...
| 手段 | 预期效果 | 风险 |
|------|----------|------|
| 降低 `--llm-call-chain-max-chars`（如 12000→6000） | 减少输入 token，加快推理 | 调用链信息变少，可能影响文档质量 |
| 降低 `--llm-call-chain-depth`（如 3→1） | 同上 | 同上 |
